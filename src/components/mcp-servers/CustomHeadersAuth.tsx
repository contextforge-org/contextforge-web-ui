    ...
    <div className="space-y-4">
      <p className="text-sm text-neutral-600 dark:text-neutral-400">
        {intl.formatMessage({
          id:
            maxHeaders === 1
              ? "mcpServer.auth.custom.descriptionOne"
              : "mcpServer.auth.custom.descriptionMany",
        })}
      </p>
        ...
        {headers.map((header, index) => (
          <div key={header.id} className="flex items-end gap-3">
            <Field
              ...
              label={intl.formatMessage({ id: "mcpServer.auth.custom.keyLabel" })}
            >
              {(controlProps) => (
                <Input
                  {...controlProps}
            ...
            <Field
              ...
              className="flex-1"
              labelProps={{ className: "text-neutral-900 dark:text-neutral-100" }}
              label={intl.formatMessage({ id: "mcpServer.auth.custom.valueLabel" })}
            >
              {(controlProps) => (
                <Input
                  {...controlProps}
      ...
      >
        ...
        {intl.formatMessage({ id: "mcpServer.auth.custom.addHeader" })}
      </Button>
    </div>
  );
}
